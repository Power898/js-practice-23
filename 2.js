...
class Group {static #groups = {};constructor(name) { this.name = name;}  static create(name) {
    if (!this.#groups[name]) { this.#groups[name] = new Group(name);} return this.#groups[name];}}
...
class Product {constructor(name, group) {this.name = name;this.group = group;}display() {
