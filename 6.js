...
class Basket {constructor(discountPlan){this.discountPlan = discountPlan;  this.goods=[];}
addGood(good){ this.goods.push(good);}
calculateTotalPrice(){const price=this.goods.reduce((acc, prod) => acc + prod.price, 0); 
...
class RegularDiscountPlan extends Basket {applyDiscount(price){ return price * 0.9;}
...
class VIPDiscountPlan extends Basket {applyDiscount(price){ return price * 0.8;}
  // Робимо метод applyDiscount, що приймає ціну price як параметр
  // Повертає ціну з урахуванням знижки в 20% price * 0.8
}

// Клас NewClientDiscountPlan: стратегія знижки для нових клієнтів
class NewClientDiscountPlan extends Basket { applyDiscount(price){ return price * 0.95;}
  // Робимо метод applyDiscount, що приймає ціну price як параметр
