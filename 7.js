...
class Employee {constructor(name, position, salary){ this.name=name; this.position= position; this.salary= salary;}
...
class EmployeeGroup {employees = []; addEmployee(employee) { this.employees.push(employee);}
...
class EmployeeIterator { #employees = null; #currentIndex =0;  constructor(employeeGroup) {this.#employees = employeeGroup.employees;}
#hasNext() { return this.#currentIndex < this.#employees.length;} 
next() {if (this.#hasNext()) {const name = this.#employees[this.#currentIndex].name; this.#currentIndex++; return name;}}
list() {return this.#employees.map((employee) => employee.name).join(", ");}
